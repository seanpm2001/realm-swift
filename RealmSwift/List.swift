...
public struct GeoPoint {
    ...
    public init(_ latitude: Double, _ longitude: Double) { fatalError() }
...
public protocol GeoShape {}

// Cannot be used within a Realm model, only to be used within Query.
public struct GeoBox: GeoShape {
    private(set) var bottomLeftCorner: GeoPoint
    private(set) var topRightCorner: GeoPoint

    // Using (top, left, bottom, right)
    public init(top: Double, left: Double, bottom: Double, right: Double) { fatalError() }
    // Using (bottomLeftCorner, topRightCorner)
    public init(bottomLeftCorner: GeoPoint, topRightCorner: GeoPoint) { fatalError() }
}

// Cannot be used within a Realm model, only to be used within Query.
public struct GeoPolygon: GeoShape {
    private(set) var outerRing: [GeoPoint]
    private(set) var holes: [[GeoPoint]] = []

    public init(outerRing: [GeoPoint], holes: [[GeoPoint]]) { fatalError() }
    public init(outerRing: [GeoPoint], holes: [GeoPoint]...) { fatalError() }
}
...
public struct Distance {
    ...
    public static func fromKilometers(_ kilometers: Double) -> Distance { fatalError() }
    // American miles, i.e. 1.609344 km pr. mile
    public static func fromMiles(_ miles: Double) -> Distance { fatalError() }
    // θ (radians) = d (kilometers) / 6378,1 km
    public static func fromRadians(_ radians: Double) -> Distance { fatalError() }

    public var asKilometers: Distance { fatalError() }
    public var asMiles: Distance { fatalError() }

    private init(radians: Double) { fatalError() }
}

// Cannot be used within a Realm model, only to be used within Query.
public struct GeoSphere: GeoShape {
    private(set) var center: GeoPoint
    private(set) var radius: Double

    // Using radius in radians
    public init(center: GeoPoint, radiusInRadians: Double) { fatalError() }
    // Using distance
    public init(center: GeoPoint, radius: Distance) { fatalError() }
}

public extension Query where T: EmbeddedObject {
    // Use geoWithin function to query over a location inside Swift Type Safe Query API, using a `GeoShape`.
    func geoWithin<U: GeoShape>(_ value: U) -> Query<Bool> {
        fatalError()
    }
}
...
class Person: Object {
    ...
    convenience init(name: String, location: Location) { fatalError() }
}
...
func test() throws {
    let realm = try Realm()
    try realm.write {
        realm.add(Person(name: "Diana", location: Location(latitude: 41.385063, longitude: 2.173404)))
    }

    // Using (top, left, bottom, right)
    let box = GeoBox(top: 1.0, left: 0.0, bottom: 0.0, right: 1.0)
    // Using (bottomLeftCorner, topRightCorner)
    let box1 = GeoBox(bottomLeftCorner: GeoPoint(0.0, 0.0), topRightCorner: GeoPoint(1.0, 1.0))
    // Using Swift Type Safe Query
    let resultsBox = realm.objects(Person.self).where { $0.location.geoWithin(box) }
    // Using NSPredicate format
    let resultsPredicateBox = realm.objects(Person.self).filter("location geoWithin $0", box)

    // A polygon without any holes
    let polygon = GeoPolygon(outerRing: [GeoPoint(-73.99, 40.73), GeoPoint(-73.98, 40.76)])
    // A polygon with holes
    let polygon2 = GeoPolygon(outerRing: [GeoPoint(0, 0), GeoPoint(1, 0)],
                              holes: [[GeoPoint(0, 0), GeoPoint(0, 0)],
                                      [GeoPoint(0, 0), GeoPoint(0, 0)]])
    let polygon3 = GeoPolygon(outerRing: [GeoPoint(0, 0), GeoPoint(1, 0)],
                              holes: [GeoPoint(0, 0), GeoPoint(0, 0)],
                                      [GeoPoint(0, 0), GeoPoint(0, 0)])
    // Using Swift Type Safe Query
    let resultsPolygon = realm.objects(Person.self).where { $0.location.geoWithin(polygon) }
    // Using NSPredicate format
    let resultsPredicatePolygon = realm.objects(Person.self).filter("location geoWithin $0", polygon)

    // Using radians
    let sphere = GeoSphere(center: GeoPoint(73.99, 40.73), radiusInRadians: 2.5)
    // Using Radius
    let sphere2 = GeoSphere(center: GeoPoint(73.99, 40.73), radius: Distance.fromKilometers(10))
    // Using Swift Type Safe Query
    let resultsSphere = realm.objects(Person.self).where { $0.location.geoWithin(sphere) }
    // Using NSPredicate format
    let resultsPredicateSphere = realm.objects(Person.self).filter("location geoWithin $0", sphere)
}
